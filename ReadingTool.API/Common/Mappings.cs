#region License
// Mappings.cs is part of ReadingTool.API
// 
// ReadingTool.API is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
// 
// ReadingTool.API is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Affero General Public License for more details.
// 
// You should have received a copy of the GNU Affero General Public License
// along with ReadingTool.API. If not, see <http://www.gnu.org/licenses/>.
// 
// Copyright (C) 2012 Travis Watt
#endregion

using AutoMapper;
using ReadingTool.API.Areas.V1.Models;
using ReadingTool.Entities;

namespace ReadingTool.API.Common
{
    public static class Mappings
    {
        public static void CreateMappings()
        {
            Mapper.CreateMap<Language, LanguageModel>()
                .ForMember(x => x.Created, y => y.MapFrom(z => z.Created.ToString("yyyyMMddHHmmss")))
                .ForMember(x => x.Modified, y => y.MapFrom(z => z.Modified.ToString("yyyyMMddHHmmss")))
                ;

            Mapper.CreateMap<Item, ItemModel>()
                .ForMember(x => x.Created, y => y.MapFrom(z => z.Created.ToString("yyyyMMddHHmmss")))
                .ForMember(x => x.Modified, y => y.MapFrom(z => z.Modified.ToString("yyyyMMddHHmmss")))
                ;

            Mapper.CreateMap<Item, GroupItemModel>()
                .ForMember(x => x.Url, y => y.MapFrom(z => z.ShareUrl ? z.Url : string.Empty))
                ;

            Mapper.CreateMap<Word, WordModel>()
                .ForMember(x => x.Created, y => y.MapFrom(z => z.Created.ToString("yyyyMMddHHmmss")))
                .ForMember(x => x.Modified, y => y.MapFrom(z => z.Modified.ToString("yyyyMMddHHmmss")))
                ;

            Mapper.CreateMap<Group, GroupModel>();
            ;
        }
    }
}